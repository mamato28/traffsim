...
for index, (rect, speed, other_rect, speed_range) in enumerate(rectangles):
    thread = threading.Thread(target=move_rect, args=(rect, speed, other_rect, canvas_width, speed_range, font, speed_texts, index, WHITE, rectangles))
...
while True:
    ...
    for index, (rect, speed, other_rect, speed_range) in enumerate(rectangles):
        if not threads[index].is_alive():  # check if the thread has terminated
            if rect.x <= 0:  # check the condition (ABC)
                thread = threading.Thread(target=move_rect, args=(rect, speed, other_rect, canvas_width, speed_range, font, speed_texts, index, WHITE, rectangles))
